...
//
// The upstream v0.6.4 source references a missing callback helper here. The
// application never supplies a prepare callback, so the vendored fix passes
// nil and preserves the public API.
func (v *Seat) Grab(window *Window, capabilities SeatCapabilities, owner_events bool, cursor *Cursor, event *Event, prepare_func GrabPrepareFunc, prepare_func_data C.gpointer) GrabStatus {
    return GrabStatus(C.gdk_seat_grab(v.native(), window.native(), C.GdkSeatCapabilities(capabilities), gbool(owner_events), cursor.native(), event.native(), nil, prepare_func_data))
}
